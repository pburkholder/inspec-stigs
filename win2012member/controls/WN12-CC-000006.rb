# encoding: utf-8
# copyright: 2016, you
# license: All rights reserved
# date: 2013-07-25
# description: Microsoft Windows Server 2012 Security Technical Implementation Guide (STIG) is published as a tool to improve the security of Department of Defense (DoD) information systems. Comments or proposed revisions to this document should be sent via e-mail to the following address: disa.letterkenny.FSO.mbx.stig-customer-support-mailbox@mail.mil.
# impacts

title 'WN12-CC-000006 - All Direct Access traffic must be routed through the internal network.'

control 'WN12-CC-000006' do
  impact 0.1
  title 'All Direct Access traffic must be routed through the internal network.'
  desc '
Routing all Direct Access  traffic through the internal network allows monitoring and prevents split tunneling.
'
  tag 'stig','WN12-CC-000006'
  tag severity: 'low'
  tag checkid: 'C-WN12-CC-000006_chk'
  tag fixid: 'F-WN12-CC-000006_fix'
  tag version: 'WN12-CC-000006'
  tag ruleid: 'WN12-CC-000006_rule'
  tag fixtext: '
Configure the policy value for Computer Configuration -> Administrative Templates -> Network -> Network Connections -> "Route all traffic through the internal network" to "Enabled: Enabled State".
'
  tag checktext: '
If the following registry value does not exist or is not configured as specified, this is a finding:

Registry Hive: HKEY_LOCAL_MACHINE
Registry Path: \Software\Policies\Microsoft\Windows\TCPIP\v6Transition\

Value Name: Force_Tunneling

Type: REG_SZ
Value: Enabled
'

# START_DESCRIBE WN12-CC-000006
  # describe file('/etc') do
  #   it { should be_directory }
  # end
# END_DESCRIBE WN12-CC-000006

end
